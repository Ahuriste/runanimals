    ...
    def __str__(self) -> str:
        if self.__return_type == "json":
            return json.dumps({
                ...
                self.alt,
                "tooltip":
                self.tooltip_format.format(**{"percentage": self.percentage}),
                "class":
                self.cls,
                "percentage":
                self.percentage,
...
animal = user_config["animal"]["cpu"]
icons = config.get("icons", "")

if isinstance(icons, str):
    icons = list(icons)
icons = list(map(str, icons))
icons = [chr(i) for i in range(runners[animal][0]+2,runners[animal][1]+1)]# runners[animal][1]+1)]
